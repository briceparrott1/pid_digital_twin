...
- Watch for a GROUP heading: sometimes one underlined name covers a pair of installed-in-parallel
  ...
  "metadata." If only the heading is visible and neither unit symbol appears in this crop at all,
  do NOT report the heading as a node — note it in "uncertainty" instead.
...
- "type": exactly one of these five categories — "equipment", "instrument", "junction",
  ...
  of these five.
- "metadata": a dict of any other readable details (e.g. size, rating, setpoint) — use {} if none.
  If this component has a separate underlined-name metadata listing elsewhere in this crop (see
  "What you will see" above), include those details here too — it's the same node, not a second one.
...
### Example 7a — a group heading covering two individually-tagged units

[Description: a crop showing the underlined heading "F-715 A & B / PARTICULATE FILTER SEPARATOR"
with shared specs listed beneath it, and two separate vessel symbols below, each with its own short
tag: "F-715A" on the left vessel, "F-715B" on the right vessel. Both vessels fully visible, no edges
touched, no lines connecting them in this crop.]

Output:
{
  "nodes": [
    {"id": "F-715A", "type": "equipment", "metadata": {"description": "PARTICULATE FILTER SEPARATOR"}},
    {"id": "F-715B", "type": "equipment", "metadata": {"description": "PARTICULATE FILTER SEPARATOR"}}
  ],
  "edges": [],
  "split_connections": [],
  "split_nodes": [],
